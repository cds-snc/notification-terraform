...
def main():
    ...
    parser.add_argument(
        "--push",
        help="push output to s3 (default: save locally instead)",
        action="store_true",
        default=False,
    )
    ...
    for key in [
        ...
        if csv_file.key.endswith(".gz") or csv_file.key.endswith(".csv")
    ]:
        print(f"\nProcessing {key}")
        df = read_csv_from_s3(s3, args.in_bucket, key).sort_values(by="PublishTimeUTC")
        ...
        if args.push:
            ...
            s3.Object(args.out_bucket, key).put(
                Body=csv_buffer.getvalue(), ContentEncoding="gzip"
            )

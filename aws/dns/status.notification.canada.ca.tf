resource "aws_route53_zone" "status-notification-canada-ca" {
  count = var.env == "production" ? 1 : 0
  name  = "status.notification.canada.ca"
}

resource "aws_route53_record" "status-notification-canada-ca-ACM-cname" {
  count   = var.env == "production" ? 1 : 0
  zone_id = aws_route53_zone.status-notification-canada-ca[0].zone_id

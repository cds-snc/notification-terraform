...
resource "aws_rds_cluster_instance" "notification-canada-ca-server-instances" {
  count                        = 3
  identifier                   = "notification-canada-ca-${var.env}-instance-${count.index}"
  cluster_identifier           = aws_rds_cluster.notification-canada-ca-server.id
...
resource "aws_rds_cluster" "notification-canada-ca-server" {

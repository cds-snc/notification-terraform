...
resource "null_resource" "build_heartbeat_docker_image" {
  ...
  provisioner "local-exec" {
    command = "pushd /var/tmp/notification-lambdas/ && docker build -t ${aws_ecr_repository.heartbeat.repository_url}:bootstrap -f /var/tmp/notification-lambdas/heartbeat/Dockerfile . && popd"
  }
...
resource "null_resource" "build_system_status_docker_image" {
  ...
  provisioner "local-exec" {
    command = "pushd /var/tmp/notification-lambdas/ && docker build -t ${aws_ecr_repository.system_status.repository_url}:bootstrap -f /var/tmp/notification-lambdas/system_status/Dockerfile . && popd"
  }
}

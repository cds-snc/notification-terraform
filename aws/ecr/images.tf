...
resource "null_resource" "build_github_arc_runner_docker_image" {
  ...
  provisioner "local-exec" {
    command = "docker build -t ${aws_ecr_repository.github_arc_runner.repository_url}:bootstrap -f ./github-runner/Dockerfile ."
  }
}
...
resource "null_resource" "push_github_arc_runner_docker_image" {
  ...
  provisioner "local-exec" {
    command = "docker push ${aws_ecr_repository.github_arc_runner.repository_url}:bootstrap"
  }

  ...
  dashboard_body = <<EOF
{
    "widgets": [
        ...
        },
        {
            ...
            "properties": {
                "metrics": [
                    [ "ContainerInsights/Prometheus", "kube_deployment_status_replicas_available", "namespace", "notification-canada-ca", "ClusterName", "notification-canada-ca-${var.env}-eks-cluster", "deployment", "notify-celery-sms-send-primary", { "region": "${var.region}", "label": "notify-celery-sms-send-primary" } ],
                    [ "ContainerInsights/Prometheus", "kube_deployment_status_replicas_available", "namespace", "notification-canada-ca", "ClusterName", "notification-canada-ca-${var.env}-eks-cluster", "deployment", "notify-celery-sms-send-scalable", { "region": "${var.region}", "label": "notify-celery-sms-send-scalable" } ]
                ],
                "sparkline": true,
                "view": "singleValue",
                "region": "${var.region}",

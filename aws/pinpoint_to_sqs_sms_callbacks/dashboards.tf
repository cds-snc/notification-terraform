        ...
        },
        {
            ...
            "properties": {
                "metrics": [
                    ...
                    [ "ContainerInsights/Prometheus", "kube_deployment_status_replicas_available", "namespace", "notification-canada-ca", "ClusterName", "${aws_eks_cluster.notification-canada-ca-eks-cluster.name}", "deployment", "celery-sms-send-scalable", { "region": "${var.region}", "label": "celery-sms-send-scalable" } ]
                ],
                "sparkline": true,
                "view": "singleValue",
                "region": "${var.region}",

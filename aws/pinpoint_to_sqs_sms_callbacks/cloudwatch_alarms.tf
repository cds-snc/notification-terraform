...
resource "aws_cloudwatch_metric_alarm" "logs-1-500-error-1-minute-warning-pinpoint_to_sqs_sms_callbacks-api-us-west-2" {
  ...
  count               = var.cloudwatch_enabled ? 1 : 0 && var.env != "production" ? 1 : 0
  alarm_name          = "logs-1-500-error-1-minute-warning-pinpoint_to_sqs_sms_callbacks-api"
  alarm_description   = "One 500 error in 1 minute for pinpoint_to_sqs_sms_callbacks api"
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = aws_cloudwatch_log_metric_filter.pinpoint_us_to_sqs_sms_callbacks-500-errors-api[0].metric_transformation[0].name
  namespace           = aws_cloudwatch_log_metric_filter.pinpoint_us_to_sqs_sms_callbacks-500-errors-api[0].metric_transformation[0].namespace
  period              = "60"
  statistic           = "Sum"
  threshold           = 1
  treat_missing_data  = "notBreaching"
...
resource "aws_cloudwatch_metric_alarm" "lambda-image-pinpoint-delivery-receipts-errors-warning-us-west-2" {
  ...
  ok_actions          = [var.sns_alert_ok_arn_us_west_2]
  dimensions = {
    FunctionName = module.pinpoint_to_sqs_sms_callbacks_us_west_2[0].function_name
  }
}
...
resource "aws_cloudwatch_metric_alarm" "pinpoint-sms-success-rate-warning-us-west-2" {
  ...
  metric_query {
    id = "successes"
    metric {
      metric_name = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-successes[0].metric_transformation[0].name
      namespace   = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-successes[0].metric_transformation[0].namespace
      period      = 60 * 60 * 12
      stat        = "Sum"
    }
  }
  ...
  metric_query {
    id = "failures"
    metric {
      metric_name = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-failures[0].metric_transformation[0].name
      namespace   = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-failures[0].metric_transformation[0].namespace
      period      = 60 * 60 * 12
      stat        = "Sum"
    }
  }
...
resource "aws_cloudwatch_metric_alarm" "pinpoint-sms-blocked-as-spam-warning-us-west-2" {
  ...
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-blocked-as-spam[0].metric_transformation[0].name
  namespace           = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-blocked-as-spam[0].metric_transformation[0].namespace
  period              = 60 * 60 * 12
  statistic           = "Sum"
  threshold           = 10
  alarm_actions       = [var.sns_alert_warning_arn_us_west_2]
...
resource "aws_cloudwatch_metric_alarm" "pinpoint-sms-rate-exceeded-warning-us-west-2" {
  ...
  comparison_operator = "GreaterThanOrEqualToThreshold"
  evaluation_periods  = "1"
  metric_name         = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-rate-exceeded[0].metric_transformation[0].name
  namespace           = aws_cloudwatch_log_metric_filter.pinpoint-us-sms-rate-exceeded[0].metric_transformation[0].namespace
  period              = 60 * 5
  statistic           = "Sum"
  threshold           = 1
  alarm_actions       = [var.sns_alert_warning_arn_us_west_2]

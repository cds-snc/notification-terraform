...
resource "aws_cloudwatch_log_group" "pinpoint_us_to_sqs_sms_callbacks_log_group" {
  provider = aws.us-west-2
...
resource "aws_cloudwatch_log_metric_filter" "pinpoint_us_to_sqs_sms_callbacks-500-errors-api" {
  ...
  name           = "pinpoint_to_sqs_sms_callbacks-500-errors-api"
  pattern        = "\"\\\"levelname\\\": \\\"ERROR\\\"\""
  log_group_name = "/aws/lambda/${module.pinpoint_to_sqs_sms_callbacks_us_west_2.function_name}"

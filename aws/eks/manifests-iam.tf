...
#
# CERT-MANAGER (ACME DNS-01 Route53)
#

data "aws_iam_policy_document" "assume_role_policy_cert_manager" {
  statement {
    actions = ["sts:AssumeRoleWithWebIdentity"]
    effect  = "Allow"
...
resource "aws_iam_role" "cert_manager_route53" {
  name               = "cert-manager-route53-eks-role"
  assume_role_policy = data.aws_iam_policy_document.assume_role_policy_cert_manager.json
}

resource "aws_iam_policy" "cert_manager_route53_dns01" {
  name = "cert-manager-route53-dns01-policy"

  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        Sid    = "AssumeDnsRoleForAcme"
        Effect = "Allow"
        Action = ["sts:AssumeRole"]
        Resource = [
          var.cert_manager_route53_role_arn
        ]
      }
    ]
  })
  ...
  lifecycle {
    precondition {
      condition     = trimspace(var.cert_manager_route53_role_arn) != ""
      error_message = "cert_manager_route53_role_arn must be set to the delegated Route53 DNS role ARN."
    }
  }
}

resource "aws_iam_role_policy_attachment" "cert_manager_route53_dns01" {
  role       = aws_iam_role.cert_manager_route53.name
  policy_arn = aws_iam_policy.cert_manager_route53_dns01.arn
}

...
resource "aws_iam_policy" "signoz_s3_permissions" {
  ...
  path   = "/"
  policy = data.aws_iam_policy_document.signoz_api_lambda_s3_permissions.json
}

resource "aws_iam_role_policy_attachment" "signoz_s3_permissions" {
  role       = aws_iam_role.api.name
  policy_arn = aws_iam_policy.signoz_s3_permissions.arn
}
...
resource "aws_lambda_function" "signoz_log_forwarder" {
  ...
  layers = [aws_lambda_layer_version.signoz_dependencies.arn]

  # Optional: Add source code hash for the function code
...
resource "aws_lambda_permission" "signoz_s3_invoke" {
  ...
  function_name = aws_lambda_function.signoz_log_forwarder.function_name
  principal     = "s3.amazonaws.com"
  source_arn    = module.signoz_api_lambda_logs.s3_bucket_arn
}

# S3 bucket notification to trigger the Lambda function
resource "aws_s3_bucket_notification" "signoz_logs_trigger" {
  bucket = module.signoz_api_lambda_logs.s3_bucket_id

  lambda_function {
    lambda_function_arn = aws_lambda_function.signoz_log_forwarder.arn

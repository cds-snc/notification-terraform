...
resource "aws_wafv2_regex_pattern_set" "re_api" {
  ...
  regular_expression {
    regex_string = "/notifications.*|/organisation.*|/organisations.*|/platform-stats.*|/provider-details.*|/service.*|/static.*|/user.*|/v1/(traces|metrics|logs).*|/v2.*|/cache-clear|/support/find-ids|/newsletter.*|/v3.*"
  }
